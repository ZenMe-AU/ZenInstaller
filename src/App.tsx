...
export default function AppDashboard() {
  ...
  const [expanded, setExpanded] = useState<Record<CardId, boolean>>(
    { auth: true, repo: true, pr: true, env: true, status_update: true, stages: true },
  );
    ...
    <>
      ...
      <Box sx={{ minHeight: "100vh", background: "#f8fafc", color: "#0f172a", fontFamily: "'IBM Plex Sans', sans-serif" }}>
        ...
        <Box sx={{ maxWidth: 860, mx: "auto", px: 4, py: 5 }}>
          {/* Intro */}
          <Box sx={{ background: "#ffffff", borderRadius: "12px", border: "1px solid #e2e8f0",
            px: 3, py: 2.5, mb: 3, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
          ...
          <Connector>
            <LoginStep status={auth.status} expanded={expanded.auth} onToggle={() => toggle("auth")}
              authLoading={auth.authLoading} user={auth.user} onLogin={auth.onLogin} onLogout={auth.onLogout} />

            <RepoStep status={repo.status} expanded={expanded.repo} onToggle={() => toggle("repo")}
              accounts={repo.accounts} selectedAccount={repo.selectedAccount} onAccountChange={repo.setSelectedAccount}
              repos={repo.repos} selectedRepo={repo.selectedRepo} onRepoChange={repo.setSelectedRepo}
              templateStatus={repo.templateStatus} templateName={repo.templateName} defaultTemplateRepo="ZenMe-AU/ZBCorpArchitecture"
            ...
            <StatusUpdateStep status={effectiveStatusUpdateStatus} expanded={expanded.status_update} onToggle={() => toggle("status_update")}
              ...
              runError={plan.runError} lastRunId={plan.lastRunId}
              repoFullName={repo.repoFullName} workflowId={repo.pipeline.workflowId} />
            ...
            {!plan.hasPlan && repo.isCloneRepo && (
              <Box sx={{ display: "flex", alignItems: "center", gap: 2, my: 1 }}>
                ...
                <Typography sx={{ fontSize: "0.65rem", letterSpacing: "0.15em", color: "#94a3b8",
                  textTransform: "uppercase", fontFamily: "'IBM Plex Mono', monospace" }}>No status file found</Typography>
            ...
            {repo.isCloneRepo && plan.isStale && (
              <Box sx={{ display: "flex", alignItems: "center", gap: 2, my: 1.5 }}>
                ...
                <Typography sx={{ fontSize: "0.65rem", letterSpacing: "0.15em", color: "#d97706",
                  textTransform: "uppercase", fontFamily: "'IBM Plex Mono', monospace", whiteSpace: "nowrap" }}>
            ...
            {repo.pipeline.stages.map((stageDef) => {
              ...
              const varsMismatch = plan.deployedEnv !== null &&
                hasVariableDiff(stageDef.prerequisites, env.presentVariableValues, plan.deployedEnv);
              const onDeploy = effectiveStatus === "success" && stage.runId
                ? async () => { if (env.selectedEnv) await plan.deployStage({ stageDef, stage, envName: env.selectedEnv.name, pr: pr.selectedPR, branches: repo.branches }); }
                : undefined;
              return (
                <StageStep key={stageDef.key}
                  ...
                  disabled={!repo.isCloneRepo} stageDef={stageDef} stage={stage} summary={summary}
                  deployDisabled={plan.isStale || varsMismatch} deployedEnv={plan.deployedEnv}
                  variableValues={env.presentVariableValues} cardStatus={cardStatus}
                  account={repo.selectedAccount} repoName={repo.selectedRepo?.name || ""}
                  selectedEnv={env.selectedEnv} onVariableConfirmed={env.onVariableConfirmed}
                  onDeploy={onDeploy} onPlanSummary={(s) => plan.setStageSummary(stageDef.key, s)} />
              );
            })}
          </Connector>
        </Box>
