...
code --install-extension ./tools/playwright-coverage-vscode/dist/zeninstaller-playwright-coverage-0.1.4.vsix --force
```
...
spec editor title, or run **Playwright: Run with Coverage**. This action
runs all tests beneath a selected directory, or the entire containing spec for a file or test item, through a visible VS Code task. Mock files and directories
...
Coverage actions open a visible browser (`--headed`). Live test progress,
pass/fail ticks, durations and debug output appear under **Playwright Coverage**
in VS Code's Testing / Test Results views. These results belong to the local
coverage extension; they do not update the official Playwright controller's rows.
The generated coverage is attached to the same run when reporting finishes.
...
coverage icon is a separate file/directory command. Reports imported by this
