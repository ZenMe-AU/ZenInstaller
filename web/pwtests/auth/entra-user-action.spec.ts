        ...
        test(
          ...
          async ({ browser }, testInfo) => {
            ...
            try {
              ...
              await expect(
                userTable,
                "Expected the Entra-user table to load.",
              ).toBeVisible({
                timeout: 45_000,
              })
              .catch(async (err) => {
              ...
              for (const target of targets) {
                await test.step(
                  ...
                  async () => {
                    ...
                    await expect(
                      targetRow,
                      `Expected one Entra-user row for ${target.email}.`,
                    ).toHaveCount(
                      1,
                      {
                        timeout: 30_000,
                    ...
                    await expect(
                      targetRow,
                      `Expected the row for ${target.email} to be visible.`,
                    ).toBeVisible();
                    ...
                    await expect(createAccessPassButton,`Expected the Create Access Pass button for ${target.email} to be enabled.`,
                    ).toBeEnabled();
                  },
                );
              }
